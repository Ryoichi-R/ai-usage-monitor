...
AI Usage Monitor は `WM_DISPLAYCHANGE`、`SPI_SETWORKAREA`、`SystemEvents.DisplaySettingsChanged`、DPI変更を同一の表示変更waveへ集約し、最後の通知から750ms、またはwave開始から2秒で最新のmonitor working areaを再取得します。HWND hookへ届かない回転経路は `SystemEvents.DisplaySettingsChanged` が補い、重複通知はdebounceされます。自由配置は保存済みfractionを新しい作業領域へ再適用し、プリセット配置は既存のanchorと余白を再適用します。

Win32のmonitor矩形と情報領域のfraction取得は物理pixel、Coreの配置計算は対象monitorのeffective DPIで変換したDIPです。表示モード、UI倍率、設定schema、保存fraction自体は自動reflowで変更しません。回転直後のstale working areaを排除するため、通常waveは成否にかかわらず1回だけ750ms後の独立settle retryを行います。各preset/custom配置は `SetWindowPos` 後の実HWND矩形を再取得し、LayoutTransform、edge fade、DPI丸めを含む実情報領域をworking area内へ物理pixelで最終clampします。retry waveからの連鎖retryは行いません。
