...
echo.
if "%EXIT_CODE%"=="3" (
  echo AI Usage Monitor ARM64 rebuild was cancelled. No files were generated.
  pause
  exit /b 3
)
if not "%EXIT_CODE%"=="0" (
  echo AI Usage Monitor ARM64 rebuild failed. Review the error above.
  pause
  exit /b %EXIT_CODE%
)

echo AI Usage Monitor ARM64 rebuild completed successfully.
pause
exit /b 0
