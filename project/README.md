...
既定では幅280 DIP、右上、常に手前、クリック透過で表示します。通知領域アイコンの「設定…」は「全体設定」「表示位置」「CODEX」「CLAUDE CODE」の左タブ式です。Windowsログオン時の自動起動、CodexとClaude Codeそれぞれの使用上限表示、追加利用額、クレジット残高、表示モニター、四隅、端からの余白、Codex実行ファイル、更新間隔を変更できます。金額表示は既定OFFです。表示モニターはWindowsのモニター固有IDで記憶するため、接続し直して `DISPLAYn` の番号が変わっても同じモニターへ表示します。記憶したモニターが未接続の間と「自動（プライマリ）」ではプライマリモニターへ表示します。起動中にもう一度起動すると、ウィジェットを再表示して通知領域に起動中であることを知らせます。
...
`artifacts/`はローカル生成物専用で、source管理および通常の公開候補には含めません。readiness確認前のx64／ARM64候補は`binary-local-only`として扱い、`internal-test`と`readiness-blocked`を明示してください。公開Release assetへ登録できるのは、対象候補のSHA-256と実機受入記録が揃った`binary-release`だけです。raw usage payloadは保存せず、テストfixture以外を配布物へ含めません。詳細は[release/README.md](../release/README.md)を参照してください。
...
「表示モード」では、標準表示と縮小表示（主要な使用率のみ）を切り替えられます。既定値は標準表示です。縮小表示は画面幅を150 DIP（内容幅134 DIP）に抑え、標準表示と同じ縦方向の行間を保ちながら、provider／Codexアカウント名、最終取得時刻、利用期間、残り割合、RESET、状態色、利用率バーを表示します。文字は標準表示より少し小さく、左右の余白と補助項目を削減しています。追加利用額とクレジット残高を確認する場合は標準表示へ切り替えてください。設定画面の保存、または通知領域アイコンの「表示モード」から切り替えでき、選択は再起動後も保持されます。表示倍率は両モードへ一体適用されます。

「外観」タブでは、メインウィジェットのフォント、前景色、補助文字色、アクセント色、警告色、危険色、背景色、透明度を変更できます。背景の塗りつぶし方式は「単色」または「四方の端を透明化」から選べます。背景を他ウィンドウの背面に置く設定は、背景を有効にし、透明度を正にし、「常に手前に表示する」を有効にした場合に利用できます。設定の反映には「保存」が必要です。

「常に手前に表示する」が有効な場合、通常の対話デスクトップ上で外部ウィンドウの前面化、移動完了、非表示からの再表示、デスクトップ復帰を検知し、フォーカスを奪わずウィジェットのTopMost順序を再適用します。TopMostウィンドウ同士の順序をイベント後に回復する機能であり、他アプリが継続的に再昇格する場合の無限競争は行いません。UACのセキュアデスクトップ、ロック画面、別セッション、排他的全画面を覆うことはありません。設定画面などのowned dialogは所有関係を維持してウィジェットより上位に表示されます。

TopMost hookの登録または復旧が一時的に失敗した場合は、アプリを停止せず次の設定適用・再表示・正規WinEventで再試行します。復旧機能が利用できない間は通知領域アイコンのTooltipに「TopMost復旧無効」と表示します。foregroundが変化しないまま表示されるTopMost popupはOSイベントで即時検知できない既知の限界です。
